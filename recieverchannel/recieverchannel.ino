...
// to a Flask server via Wi-Fi.
//
// Uses ArduinoJson for robust JSON payload creation.
...
const char* flaskServerUrl = "http://10.53.159.152:5000/log_data"; //computer ip
const char* commandServerBaseUrl = "http://10.53.159.152:5000/get-command/"; // Base URL for commands */
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        if (httpResponseCode > 0) {
            ...
            if (httpResponseCode == HTTP_CODE_OK || httpResponseCode == HTTP_CODE_MOVED_PERMANENTLY) {
                ...
                Serial.printf("Response from Flask: %s\n", payload.c_str());
            }
        } else {
...
void pollForCommandsForStation(int stationId) {
    if (WiFi.status() == WL_CONNECTED) {
        ...
        if (httpResponseCode == HTTP_CODE_OK) {
            ...
            Serial.printf("[HTTP] GET failed for station %d, error: %s\n", stationId, http.errorToString(httpResponseCode).c_str());
        }
        http.end();
    } else {
